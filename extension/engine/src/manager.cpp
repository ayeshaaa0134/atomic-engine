...
Manager::Manager(const std::string& filename, std::size_t region_size, std::size_t block_size, bool create_new)
    ...
    file_handle_ = CreateFileA(filename.c_str(), access_mode, share_mode, nullptr, creation_disp, FILE_ATTRIBUTE_NORMAL, nullptr);
    ...
    // But if it's OPEN_ALWAYS and exists, setting size might be optional but good.
    // Wait, CreateFileMapping with size will expand it.
    
    map_handle_ = CreateFileMappingA(file_handle_, nullptr, PAGE_READWRITE, size_li.HighPart, size_li.LowPart, nullptr);
