...
public:
  void insert(int key, int value) {
    ...
    // SQLite writes: Log entry (~100 bytes) + Page (4096 bytes) = ~4.2KB per
    // 8-byte update Write amplification: 4200 / 8 = 525x
...
void print_comparison_table(const std::vector<BenchmarkResult> &results) {
  ...
  std::cout << std::left << std::setw(25) << "Engine" << std::setw(20)
            << "Throughput (ops/s)" << std::setw(15) << "Latency (ms)"
  ...
  for (const auto &result : results) {
    std::cout << std::left << std::setw(25) << result.engine_name
              ...
              << result.throughput_ops_sec << std::setw(15) << std::fixed
              << std::setprecision(4) << result.latency_ms;
...
void export_results_csv(const std::vector<BenchmarkResult> &results,
  ...
  for (const auto &result : results) {
    csv << result.engine_name << "," << result.throughput_ops_sec << ","
        << result.latency_ms << "," << result.write_amplification << "\n";
  }
...
int main() {
  ...
  try {
    ...
    std::cout
        << "✓ AtomicTree achieves 1x write amplification (vs SQLite's ~500x)"
        << std::endl;
    std::cout << "✓ Performance competitive with volatile memory structures"
              << std::endl;
