...
void test_massive_insertions() {
    ...
    std::cout << "✓ Insertion complete in " << duration.count() << " ms" << std::endl;
    std::cout << "  Throughput: " << (NUM_KEYS * 1000.0 / duration.count()) << " ops/sec" << std::endl;
    ...
    for (size_t i = 0; i < NUM_KEYS; i++) {
        ...
        if (!tree.search(static_cast<int>(i), value)) {
            ...
            std::cerr << "  ERROR: Key " << i << " has wrong value: " << value << std::endl;
            errors++;
        }
...
void test_random_workload() {
    ...
    std::cout << "Executing " << NUM_OPS << " random insert operations..." << std::endl;
    auto start = high_resolution_clock::now();
    ...
    std::cout << "✓ Random insertions complete in " << duration.count() << " ms" << std::endl;
    ...
    std::cout << "✓ Verified " << verified << " keys, " << errors << " errors" << std::endl;
}
...
void test_crash_recovery() {
    ...
    {
        std::cout << "Phase 1: Creating database and inserting data..." << std::endl;
        Manager manager(db_file, region_size, 256, true);
    ...
    // Phase 2: "Crash" - reopen without create_new flag
    {
        ...
        Manager manager(db_file, region_size, 256, false); // Reopen existing
        ...
        std::cout << "  Leaked blocks recovered: " << gc.blocks_freed() << std::endl;
        ...
        if (errors == 0) {
            ...
            std::cout << "✗ Found " << errors << " errors after recovery" << std::endl;
        }
    }
}
