...
BTree::InsertResult BTree::insert_internal_node(std::uint64_t node_offset,
  ...
  if (res.did_split) {
    if (node->key_count < config_.max_keys) {
      ...
      // 4. Update Count (Atomic Commit of the insert)
      node->key_count++;
      persist(&node->key_count, sizeof(int));
      ...
      target->key_count++;
      persist(&target->key_count, sizeof(int));
...
BTree::InsertResult BTree::split_internal(std::uint64_t old_node_offset) {
  ...
  // 3. Persistent Flush of the Shadow Node
  persist(new_node, manager_->block_size());

  // 4. Shrink Old Node in-place
  // Note: For internal nodes, we shrink first then return the split result
  // for parent update. In a full B-Link implementation, we'd use a next
  // pointer for internal nodes too, but here we follow the "Atomic Split"
  // rule: Parent remains unchanged until it atomically inherits the new
  // child.
