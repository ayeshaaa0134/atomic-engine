...
static inline int* get_internal_keys(BTreeNode* node) {
    return &node->keys;
}

static inline std::uint64_t* get_internal_children(BTreeNode* node, int max_keys) {
    intptr_t keys_end = reinterpret_cast<intptr_t>(&node->keys) + max_keys * sizeof(int);
    keys_end = (keys_end + 7) & ~7; 
    return reinterpret_cast<std::uint64_t*>(keys_end);
}

static inline LeafEntry* get_leaf_entries(BTreeNode* node) {
    return &node->entries;
}

static inline std::uint64_t* get_leaf_next(BTreeNode* node, int leaf_capacity) {
...
BTree::BTree(Manager* manager, const BTreeConfig& config)
    ...
    std::uint64_t* next = get_leaf_next(root, config.leaf_capacity);
    *next = 0;
    
    persist(root, manager_->block_size());
}
...
void BTree::insert(int key, int value) {
    ...
    if (res.did_split) {
        ...
        persist(new_root, manager_->block_size());
        
        // Update root (Volatile in this object, but in real DB this goes to Superblock)
...
BTree::InsertResult BTree::insert_leaf(std::uint64_t leaf_offset, int key, int value) {
    ...
    if (leaf->key_count < config_.leaf_capacity) {
        ...
        // Flush Entry
        persist(&entries[idx], sizeof(LeafEntry));
        
        // Update Count and Flush
        leaf->key_count++;
        persist(&leaf->key_count, sizeof(int));
...
BTree::InsertResult BTree::insert_internal_node(std::uint64_t node_offset, int key, int value) {
    ...
    int idx = 0;
    while (idx < node->key_count && key >= keys[idx]) idx++;
    ...
    if (res.did_split) {
        if (node->key_count < config_.max_keys) {
            ...
            int t_idx = 0;
            while (t_idx < t_count && res.split_key >= t_keys[t_idx]) t_idx++;
            ...
            persist(target, manager_->block_size());
            return my_split;
        }
    }
    return {0, 0, false};
...
BTree::InsertResult BTree::split_leaf(std::uint64_t old_leaf_offset) {
    ...
    for(int i=0; i<total; ++i) buffer[i] = old_entries[i];
    std::sort(buffer.begin(), buffer.end(), [](const LeafEntry& a, const LeafEntry& b){
        return a.key < b.key;
    });
    ...
    // Update chain: New->Next = Old->Next
    ...
    *new_next = *old_next;
    ...
    persist(new_leaf, manager_->block_size());
    
    // Update Old Leaf: Atomic pointer update if possible, but here we just rewrite content
    // We rewrite keep_count and sorted low keys
    // B-Link consistency: Update Old->Next = New
    *old_next = new_leaf_offset;
    
    for(int i=0; i<mid; ++i) old_entries[i] = buffer[i];
...
bool BTree::search_internal(std::uint64_t node_offset, int key, int& out_value) const {
    BTreeNode* node = offset_to_node(node_offset);
    ...
    if (node->is_leaf) {
        ...
        int i = 0;
        while (i < node->key_count && key >= keys[i]) i++;
        return search_internal(children[i], key, out_value);
    }
}
bool BTree::erase(int key) { return false; }
